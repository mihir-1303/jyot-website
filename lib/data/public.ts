...
const media = (value: unknown): PublicMediaAsset => { const item = value as { _id?: unknown; objectKey?: string; sourceUrl?: string; altText?: string; caption?: string; width?: number; height?: number; variants?: { original?: { objectKey: string; width?: number; height?: number }; presentation16x9?: { objectKey: string; width?: number; height?: number } } }; const variant = item.variants?.presentation16x9; const original = item.variants?.original; const key = variant?.objectKey ?? item.objectKey ?? ""; const originalKey = original?.objectKey ?? item.objectKey ?? ""; const originalUrl = process.env.MEDIA_DELIVERY_BASE_URL && originalKey ? deliveryUrl(originalKey) : uncroppedSourceUrl(item.sourceUrl) ?? deliveryUrl(originalKey); return { id: idOf(item), url: item.sourceUrl ?? deliveryUrl(key), originalUrl, altText: item.altText ?? "", caption: item.caption, width: variant?.width ?? item.width, height: variant?.height ?? item.height, originalWidth: original?.width ?? item.width, originalHeight: original?.height ?? item.height }; };
