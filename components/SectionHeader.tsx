...
export function SectionViewAll({ href, label, dark = false }: { href: string; label: string; dark?: boolean }) {
  return <div className={`section-view-all ${dark ? "section-view-all-dark" : ""}`}><Link className="section-view-all-link" href={href}>{label} <span className="arrow" aria-hidden="true">→</span></Link></div>;
}
