...
const media = (value: unknown) => { const item = value as AnyRow | undefined; if (!item) return undefined; const variant = item.variants?.presentation16x9; const key = variant?.objectKey ?? item.objectKey ?? ""; const base = (process.env.MEDIA_DELIVERY_BASE_URL ?? "").replace(/\/$/, ""); return { url: item.sourceUrl ?? `${base}/${key}`, altText: item.altText }; };
