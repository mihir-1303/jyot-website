...
function Select({ name, label, value, options, multiple = false, required = false }: { name: string; label: string; value: string; options: Option[]; multiple?: boolean; required?: boolean }) { const [selected, setSelected] = useState(value ? value.split(",").filter(Boolean) : []); return <label className="block text-sm">{label}<select className="mt-1 w-full border p-3" name={multiple ? `${name}Select` : name} value={multiple ? selected : value} onChange={(event) => setSelected(multiple ? [...event.target.selectedOptions].map((option) => option.value) : [event.target.value])} multiple={multiple} required={required}>{!multiple && <option value="">Select {label.toLowerCase()}</option>}{options.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select>{multiple && <input type="hidden" name={name} value={selected.join(",")} />}</label>; }
