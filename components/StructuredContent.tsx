...
export function StructuredContent({ content, media = {} }: { content: unknown; media?: Record<string, PublicMediaAsset> }) {
  if (typeof content === "string") return <div className="prose max-w-none whitespace-pre-wrap leading-8">{content}</div>;
  const render = (node: Node, key: string): React.ReactNode => {
    ...
    if (node.type === "heading") { const level = node.attrs?.level === 3 ? 3 : 2; return level === 3 ? <h3 key={key}>{children}</h3> : <h2 key={key}>{children}</h2>; }
  ...
  return <div className="prose max-w-none leading-8">{render(content as Node, "content")}</div>;
}
