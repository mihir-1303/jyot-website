...
export default async function VideoPage({ params }: Props) {
  ...
  return <main className="page-shell section max-w-4xl"><Breadcrumbs items={[{ label: "Videos", href: "/videos" }, { label: item.title }]} /><JsonLd data={data} /><EditorialHeader content={videoToEditorialContent(item)} showExcerpt={false} /><div className="mt-12"><VideoPlayer video={item} /></div>{item.description && <p className="mt-8 text-lg leading-8 text-[var(--muted)]">{item.description}</p>}{collectionNavigation[0] && <CollectionNavigation {...collectionNavigation[0]} />}</main>;
}
