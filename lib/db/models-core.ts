...
const sectionSchema = new Schema({ id: { type: String, required: true }, type: { type: String, enum: ["hero", "featured", "articles", "videos", "research"], required: true }, title: { type: String, required: true }, enabled: Boolean, order: Number, content: Schema.Types.Mixed, settings: Schema.Types.Mixed }, { _id: false });
const snapshotSchema = new Schema({ sections: [sectionSchema], revision: Number, updatedAt: Date }, { _id: false });
const homepageSchema = new Schema({ key: { type: String, enum: ["homepage"], unique: true, required: true }, draft: { type: snapshotSchema, required: true }, published: { type: snapshotSchema, required: true }, draftRevision: { type: Number, default: 1 }, publishedRevision: { type: Number, default: 1 }, draftUpdatedBy: ref("User"), publishedBy: { type: Schema.Types.ObjectId, ref: "User" }, publishedAt: Date, ...audit }, opts);
