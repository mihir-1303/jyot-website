...
async function save(kind: "article" | "video" | "research", formData: FormData) {
  ...
  const publicTag = permissionBase === "articles" ? "public-articles" : permissionBase === "videos" ? "public-videos" : "public-research";
  revalidateTag(publicTag, "max"); revalidateTag(`public-${permissionBase.slice(0, -1)}:${slug}`, "max"); if (existing?.slug) revalidateTag(`public-${permissionBase.slice(0, -1)}:${existing.slug}`, "max"); revalidateTag("public-homepage", "max"); revalidateTag("public-categories", "max"); revalidatePath("/"); revalidatePath(`/${permissionBase}`); revalidatePath(`/admin/${permissionBase}`); if (existing?.slug) revalidatePath(`/${permissionBase}/${existing.slug}`); revalidatePath(`/${permissionBase}/${slug}`); revalidatePath("/search"); return { ok: true };
}
