...
const deliveryUrl = (objectKey: string) => { const base = process.env.MEDIA_DELIVERY_BASE_URL?.replace(/\/$/, ""); return base && objectKey ? `${base}/${objectKey}` : undefined; };
const uncroppedSourceUrl = (sourceUrl?: string) => sourceUrl?.replace(/([?&])fit=crop(&?)/, "$1fit=max$2");
const media = (value: unknown): PublicMediaAsset => { const item = value as { _id?: unknown; objectKey?: string; sourceUrl?: string; altText?: string; caption?: string; width?: number; height?: number; variants?: { original?: { objectKey: string; width?: number; height?: number }; presentation16x9?: { objectKey: string; width?: number; height?: number } } }; const variant = item.variants?.presentation16x9; const original = item.variants?.original; const key = variant?.objectKey ?? item.objectKey ?? ""; const originalKey = original?.objectKey ?? item.objectKey ?? ""; const sourceUrl = uncroppedSourceUrl(item.sourceUrl); const originalUrl = sourceUrl ?? deliveryUrl(originalKey); const url = item.sourceUrl ?? deliveryUrl(key) ?? originalUrl ?? ""; return { id: idOf(item), url, originalUrl, altText: item.altText ?? "", caption: item.caption, width: variant?.width ?? item.width, height: variant?.height ?? item.height, originalWidth: original?.width ?? item.width, originalHeight: original?.height ?? item.height }; };
...
const videoDto = (value: unknown): PublicVideo => { const item = value as Record<string, unknown>; const sourceType = item.sourceType as PublicVideo["sourceType"]; const mediaAsset = item.media ? media(item.media) : undefined; return { id: idOf(item), title: String(item.title), slug: String(item.slug), description: String(item.description ?? ""), thumbnail: media(item.thumbnail), provider: item.provider as PublicVideo["provider"], sourceType, videoUrl: item.externalUrl ? String(item.externalUrl) : sourceType === "r2" ? mediaAsset?.originalUrl ?? mediaAsset?.url : undefined, duration: item.duration ? String(item.duration) : undefined, author: item.author ? author(item.author) : undefined, category: category(item.category), status: "published", featured: Boolean(item.featured), publishedAt: dateOf(item.publishedAt), createdAt: dateOf(item.createdAt), updatedAt: dateOf(item.updatedAt) }; };
...
const getPublishedArticleBySlugCached = unstable_cache(async (slug: string) => { await connectToDatabase(); const value = await Article.findOne({ ...publicFilter(), slug }).populate(articlePopulate).lean().exec(); return value ? articleDto(value) : null; }, ["public-article-detail"], { revalidate: PUBLIC_DETAIL_REVALIDATE });
export const getPublishedArticleBySlug = cache((slug: string) => getPublishedArticleBySlugCached(slug));
const getPublishedVideoBySlugCached = unstable_cache(async (slug: string) => { await connectToDatabase(); const value = await Video.findOne({ ...publicFilter(), slug }).populate(videoPopulate).lean().exec(); return value ? videoDto(value) : null; }, ["public-video-detail"], { revalidate: PUBLIC_DETAIL_REVALIDATE });
export const getPublishedVideoBySlug = cache((slug: string) => getPublishedVideoBySlugCached(slug));
const getPublishedResearchBySlugCached = unstable_cache(async (slug: string) => { await connectToDatabase(); const value = await Research.findOne({ ...publicFilter(), slug }).populate(researchPopulate).lean().exec(); return value ? researchDto(value) : null; }, ["public-research-detail"], { revalidate: PUBLIC_DETAIL_REVALIDATE });
export const getPublishedResearchBySlug = cache((slug: string) => getPublishedResearchBySlugCached(slug));

const getRelatedPublishedArticlesCached = unstable_cache(async (articleId: string, categoryId: string, limit: number) => { await connectToDatabase(); const related = await Article.find({ ...publicFilter(), _id: { $ne: articleId }, category: categoryId }).sort({ publishedAt: -1 }).limit(limit).populate(articlePopulate).lean().exec(); if (related.length >= limit) return related.map(articleDto); const excludedIds = [articleId, ...related.map((item) => idOf(item))]; const fallback = await Article.find({ ...publicFilter(), _id: { $nin: excludedIds }, category: { $ne: categoryId } }).sort({ publishedAt: -1 }).limit(limit - related.length).populate(articlePopulate).lean().exec(); return [...related, ...fallback].map(articleDto); }, ["public-related-articles"], { revalidate: PUBLIC_DETAIL_REVALIDATE });
...
const getPublishedCollectionsCached = unstable_cache(async (limit: number | null): Promise<PublicCollectionSummary[]> => { await connectToDatabase(); return getHomepageCollections({ ids: [], latestLimit: limit ?? undefined }); }, ["public-collections"], { revalidate: PUBLIC_LIST_REVALIDATE });
export function getPublishedCollections(limit?: number) { return getPublishedCollectionsCached(limit ?? null); }
const getPublishedCollectionBySlugCached = unstable_cache(async (slug: string) => { await connectToDatabase(); const value = await Collection.findOne({ ...publicFilter(), slug }).populate("coverImage").populate("curator", "name slug bio").select("title slug description coverImage curator items publishedAt updatedAt seo").lean(); return value ? collectionDto(value) : null; }, ["public-collection-detail"], { revalidate: PUBLIC_DETAIL_REVALIDATE });
export const getPublishedCollectionBySlug = cache((slug: string) => getPublishedCollectionBySlugCached(slug));
...
export const getHomepageData = unstable_cache(getHomepageDataUncached, ["public-homepage"], { revalidate: PUBLIC_HOMEPAGE_REVALIDATE });
