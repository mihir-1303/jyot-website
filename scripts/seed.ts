...
import { Article, Author, Category, HomepageConfig, MediaAsset, Research, Tag, User, Video } from "../lib/db/models-core";
...
async function seed() {
  ...
  const sections = (homepageData.config.sections as any[]).map((section) => ({ ...section, content: section.content?.mode === "manual" ? { ...section.content, ids: section.content.ids.map((id: string) => [...articleDocs, ...researchDocs].find((item: any) => item.legacyId === id)?._id) } : section.content }));
