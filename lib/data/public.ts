...
const collectionArticleSelect = "title slug excerpt publishedAt coverMedia author";
const collectionResearchSelect = "title slug description publishedAt coverMedia authors";
...
const getPublishedVideosCached = unstable_cache(async (limit: number | null, categorySlug: string | null, homepage: boolean) => { await connectToDatabase(); const categoryId = await categoryIdForSlug(categorySlug ?? undefined); if (categorySlug && !categoryId) return []; const q = Video.find({ ...publicFilter(), ...(categoryId ? { category: categoryId } : {}) }).sort({ publishedAt: -1 }).select(homepage ? homepageVideoSelect : videoListSelect); if (limit) q.limit(limit); return (await q.populate(homepage ? homepageVideoPopulate : videoListPopulate).lean().exec()).map(videoDto); }, ["public-videos"], { revalidate: PUBLIC_LIST_REVALIDATE, tags: ["public-videos"] });
...
const getPublishedResearchCached = unstable_cache(async (limit: number | null, categorySlug: string | null, homepage: boolean) => { await connectToDatabase(); const categoryId = await categoryIdForSlug(categorySlug ?? undefined); if (categorySlug && !categoryId) return []; const q = Research.find({ ...publicFilter(), ...(categoryId ? { category: categoryId } : {}) }).sort({ publishedAt: -1 }).select(homepage ? homepageResearchSelect : researchListSelect); if (limit) q.limit(limit); return (await q.populate(homepage ? homepageResearchPopulate : researchListPopulate).lean().exec()).map(researchDto); }, ["public-research"], { revalidate: PUBLIC_LIST_REVALIDATE, tags: ["public-research"] });
...
const getRelatedPublishedArticlesCached = unstable_cache(async (articleId: string, categoryId: string, limit: number) => { await connectToDatabase(); const related = await Article.find({ ...publicFilter(), _id: { $ne: articleId }, category: categoryId }).sort({ publishedAt: -1 }).limit(limit).select(articleListSelect).populate(articleListPopulate).lean().exec(); if (related.length >= limit) return related.map(articleDto); const excludedIds = [articleId, ...related.map((item) => idOf(item))]; const fallback = await Article.find({ ...publicFilter(), _id: { $nin: excludedIds }, category: { $ne: categoryId } }).sort({ publishedAt: -1 }).limit(limit - related.length).select(articleListSelect).populate(articleListPopulate).lean().exec(); return [...related, ...fallback].map(articleDto); }, ["public-related-articles"], { revalidate: PUBLIC_DETAIL_REVALIDATE, tags: ["public-articles"] });
...
async function collectionItems(items: CollectionRow["items"]): Promise<PublicCollectionItem[]> {
  ...
  const [articles, research, videos] = await Promise.all([
    Article.find({ ...publicFilter(), _id: { $in: ids.article } }).select(collectionArticleSelect).populate(collectionArticlePopulate).lean(),
    Research.find({ ...publicFilter(), _id: { $in: ids.research } }).select(collectionResearchSelect).populate(collectionResearchPopulate).lean(),
    Video.find({ ...publicFilter(), _id: { $in: ids.video } }).select(collectionVideoSelect).populate(collectionVideoPopulate).lean(),
  ]);
...
const getCollectionNavigationCached = unstable_cache(async (type: "article" | "research" | "video", contentId: string) => {
  await connectToDatabase();
  const Model = type === "article" ? Article : type === "research" ? Research : Video;
  const [current, rows] = await Promise.all([
    Model.findOne({ ...publicFilter(), _id: contentId }).select("slug").lean() as unknown as Promise<{ slug?: string } | null>,
    Collection.find({ ...publicFilter(), "items.type": type, "items.contentId": contentId }).select("title slug items").lean(),
  ]);
  const matches = await Promise.all((rows as unknown as CollectionRow[]).map(async (row) => {
    const items = await collectionItems(row.items);
    const index = current?.slug ? items.findIndex((item) => item.type === type && item.slug === current.slug) : -1;
    return index >= 0 ? { collection: { title: row.title, slug: row.slug }, index, total: items.length, previous: items[index - 1], next: items[index + 1] } : null;
  }));
  return matches.filter((match): match is NonNullable<typeof match> => Boolean(match));
}, ["public-collection-navigation"], { revalidate: PUBLIC_DETAIL_REVALIDATE, tags: ["public-collections", "public-articles", "public-videos", "public-research"] });
