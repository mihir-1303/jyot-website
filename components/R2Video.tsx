...
export function R2Video({ src, poster, title }: { src: string; poster?: string; title: string }) {
  ...
  return <div className="video-player"><video controls playsInline preload="metadata" poster={poster} aria-label={`Video: ${title}`} onError={() => setFailed(true)}><source src={src} /></video></div>;
}
