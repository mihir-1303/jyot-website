...
export const homepageSectionSchema = z.discriminatedUnion("type", [
  ...
  z.object({ ...baseSection, type: z.literal("featured"), content: manualContentSchema }),
...
export const homepageSnapshotSchema = z.object({ sections: z.array(homepageSectionSchema).max(20), revision: z.number().int().nonnegative(), updatedAt: z.coerce.date() }).superRefine((snapshot, ctx) => { const ids = snapshot.sections.map((section) => section.id); const orders = snapshot.sections.map((section) => section.order); if (new Set(ids).size !== ids.length) ctx.addIssue({ code: "custom", message: "Homepage section IDs must be unique." }); if (new Set(orders).size !== orders.length) ctx.addIssue({ code: "custom", message: "Homepage section orders must be unique." }); for (const section of snapshot.sections) { if (section.type === "hero" && section.content.ids.length !== 1) ctx.addIssue({ code: "custom", message: "Hero must select exactly one item." }); if (section.type === "featured" && section.content.ids.length !== 3) ctx.addIssue({ code: "custom", message: "Featured must select one primary and two supporting items." }); if (section.type === "research" && section.content.mode === "manual" && section.content.ids.length > 2) ctx.addIssue({ code: "custom", message: "Featured Research may select at most two items." }); } });
